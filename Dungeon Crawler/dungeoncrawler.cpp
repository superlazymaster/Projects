 ...
 int main() {
   ...
   // this is to load the new file name every time the player reaches a door
   for (int current_room = 1; current_room <= total_rooms; current_room++) {
     ...
     while (true) {
       ...
       if (input != MOVE_UP && input != MOVE_LEFT && input != MOVE_DOWN &&
           input != MOVE_RIGHT && input != INPUT_STAY) {
       ...
       if (input == INPUT_STAY) {
         ...
         // we will use the player's current location and pass-by-reference to
         // find the intended next location
         nextRow = player.row;
         nextCol = player.col;
         getDirection(input, nextRow, nextCol);

         // move player to new location index, if possible, and get player
